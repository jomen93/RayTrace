...
# Define a photon       
p1 = Photon(Alpha = 1., Beta = 0.)    
p2 = Photon(Alpha = 0.0, Beta = 5.)    
...
coords1 = odeint(m.geodesics,coord01,[0,-1])

j = 1

while np.cos(coords1[j,2]) > 0.7 :
    coordloop = odeint(m.geodesics,coords1[j],[0,-0.01])
    coords1 = np.concatenate((coords1,[coordloop[1]]))
    j = j + 1

print(coords1)
print(j)
print(coords1[j,2])
print(np.cos(coords1[j,2]))
